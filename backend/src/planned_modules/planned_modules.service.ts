...
type PlannedModuleDataInput = Omit<CreatePlannedModuleDto, 'status'> & {
  status?: PlannedModuleStatusDto | PlannedModuleStatus | string;
};
...
export class PlannedModulesService {
  ...
  async update(userId: string, id: string, updatePlannedModuleDto: UpdatePlannedModuleDto): Promise<PlannedModule> {
    ...
    const status =
      ...
      (updatePlannedModuleDto.semesterId ? PlannedModuleStatus.PLANNED : existingPlannedModule.status);
    ...
    return this.prisma.plannedModule.update({
      where: { id },
      data: normalizedData as Prisma.PlannedModuleUncheckedUpdateInput,
      include: { module: true, semester: true },
    });
  }
